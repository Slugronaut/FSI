...
namespace Toolbox
{
     ...
     * 
     * Promise: Similar to the demand above. But in this case the listener isn't expected to always immediately invoke the callback. Instead
     *          it is a 'promise' to eventually invoke it when the time is appropriate. This is useful for reacting to events that then require
     *          some undetermined amount of time to complete but should not block progress in the meantime (such as doing anything that involves a network).
     */
	...
	public class InstantMessageDispatcher : IMessageDispatcher
	{
        ...
        Dictionary<Type, Delegate> Listeners = new Dictionary<Type, Delegate>(PreAllocSize);
        Dictionary<Type, MessageHandler> RuntimeListeners = new Dictionary<Type, MessageHandler>(PreAllocSize);
        Dictionary<object, IMessageDispatcher> LocalDispatches = new Dictionary<object, IMessageDispatcher>();
        HashSet<Type> UniqueMessageTypes = new HashSet<Type>();
	...
	{
		Dictionary<Type, Queue<IMessage>> PendingMessages = new Dictionary<Type, Queue<IMessage>>(PreAllocSize);
		Dictionary<Type, List<IMessage>> BufferedMessages = new Dictionary<Type, List<IMessage>>(PreAllocSize);
		...
		List<IMessage> BufferOnDelayMessages = new List<IMessage>(PreAllocSize);
        ...
        public virtual void ProcessAllPendingMessages()
		{
            ...
            var keys = PendingMessages.Keys.ToList();// SimpleKeys;
            if (keys != null)
            {
                Type key;
                //foreach (var key in PendingMessages.Keys)
                for (int i = 0; i < PendingMessages.Count; i++)
                {
                    ...
                    var queue = PendingMessages[key];
                    while (queue.Count > 0)
                    {
                        var msg = queue.Dequeue();
                        //var msgType = key; this was messing things up. somehow unity's compiler changed the rules on me when switching projects
                        var msgType = msg.GetType();

                        //BUG FIX: 5/10/2017
                        //The new multithreaded object deserialization was actually kicking in here
                        //and causes objects that subscribed to Deferred-buffered messages to actuall miss them
