...
namespace SAOT
{
    ...
    public partial class ModUserForm : Form
    {
        ...
        {
            this.UsernameTextbox.Text = DbAccess.Unsanitize(user.Name);
        ...
        private void ChangePasswordButton_Click(object sender, EventArgs args)
        {
            using (var dialog = new TaskDialog())
            {
                ...
                if (dialog.ShowDialog(this) == yesButton)
                {
                    try
                    {
                        User.UpdatePassword(CurrentUser, ModdedUser.Name, this.PasswordTextbox.Text);
                        MessageBox.Show(this, "The user '" + ModdedUser.Name + "' has had their password successfully updated.");
                    }
                    catch(Exception e)
                    {
        ...
        private void SaveButton_Click(object sender, EventArgs args)
        {
            ...
            try
            {
                User.ApplyUserSettings(CurrentUser, ModdedUser.Name, DbAccess.Sanitize(this.EmailTextbox.Text), rights);
            }
            catch (Exception e)
            {
                MessageBox.Show(this, e.Message);
            ...
            MessageBox.Show(this, "The user '" + ModdedUser.Name + "' was successfully updated.");
            ModdedUser = null;
            this.Close();
        }
    }
