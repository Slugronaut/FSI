...
namespace SAOT
{
    public partial class MaterialsListForm : Form, IProjectProvider
    {
        Project Proj;
        ...
        public MaterialsListForm(ref User user, Project proj)
        {
            Proj = proj;
            CurrentUser = new WeakReference(user);
            InitializeComponent();
            this.FormClosing += HandleClosing;

            DataGrid.SelectionMode = DataGridViewSelectionMode.CellSelect;// FullRowSelect;
            ...
            DataGrid.CellMouseDoubleClick += WindowUtils.HandleDrawingLookup;
            ...
            this.DataGrid.RowsDefaultCellStyle.BackColor = Color.LemonChiffon;
        }
        ...
        private void dataGridView1_MouseMove(object sender, MouseEventArgs e)
        {
            if ((e.Button & MouseButtons.Right) == MouseButtons.Right)
            {
                    ...
                    !dragBoxFromMouseDown.Contains(e.X, e.Y))
                {
                    ...
                    InvMovePackage.BeginMove(null, matId);
        ...
        private void createToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (User.IsUserLoggedIn(CurrentUser) && (user.IsAdmin || user.CanManageMaterials))
            {
                var form = new CreateMaterialForm(Proj, user);
                form.ShowDialog(this);
            }
            else
            {
                Dialog.Message("Insufficient access rights to modify or create materials.");
            }
        }
    }
}
